...
/**
 * ツイートフィルタモジュールのテストを実装するためのヘルパ
 */
export class TweetFilterTestHelper {
  /**
   * ツイートフィルタモジュールの初期化
   * @param moduleName ツイートフィルタモジュール名 (例: 'FilterTweetTextBayesian')
   ...
   * テスト用ツイートの取得
   */
  public static getTweet(): CrawledTweet {
    return {
      id: 0,
  ...
  protected static getMockHelper(moduleName: string, moduleSetting: any) {
    return TweetFilterHelper.factoryTest(moduleName, moduleSetting);
  }
}
