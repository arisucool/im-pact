 ...
 * https://github.com/mugifly/angular-nest/blob/master/.utils/api-client-generator-launcher.js
 */
...
class APIClientGeneratorLauncher {
  ...
  static SERVER_DIR = `${__dirname}/../packages/server`;
  static CLIENT_DIR = `${__dirname}/../packages/client`;
  ...
  async start() {
    ...
    console.log('API Client was generated to ', APIClientGeneratorLauncher.API_CLIENT_OUTPUT_PATH);
  }
  ...
  shouldGenerateApiClient(yaml) {
    ...
    if (!fs.existsSync(APIClientGeneratorLauncher.API_DOC_YAML_PATH)) return true;

    let savedHash;
    try {
      const savedSum = crypto.createHash('sha1');
      savedSum.update(fs.readFileSync(APIClientGeneratorLauncher.API_DOC_YAML_PATH).toString());
  ...
  existsApiClient() {
    return fs.existsSync(`${APIClientGeneratorLauncher.API_CLIENT_OUTPUT_PATH}/api.module.ts`);
  }
